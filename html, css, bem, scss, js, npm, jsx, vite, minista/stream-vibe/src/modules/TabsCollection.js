...
class Tabs extends BaseComponent {
    ...
    updateNavigationCSSVars(activeButtonElement = this.buttonElements[this.state.activeTabIndex]) {
        const {width, left} = activeButtonElement.getBoundingClientRect() //получаем ширину и смещение налево у кнопки
        const offsetLeft = left - this.navigationElement.getBoundingClientRect().left  //сравниваем значение смещения у кнопки и родительского элемента
                                                                                        // (получим кол-во пискселей, на которые надо сместить подложку)
